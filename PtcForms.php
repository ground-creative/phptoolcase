	...
	class PtcForms
	{
		...
		*/
		public function addElement($params)
		{
			...
			$this->_formFields[$fieldName]=array('type'=>$params['type']);
			unset($params['name']);
			unset($params['type']);
			...
			foreach($params as $k=>$v)
			{
				if($match=preg_match('/\[.*?\]/',$k,$matches)) // work with []
				{
					...
					if($this->_formFields[$fieldName]['type']!='radiogroup' && 
							$this->_formFields[$fieldName]['type']!='checkboxgroup' &&
										$this->_formFields[$fieldName]['type']!='select')
					{
						trigger_error(__CLASS__.'::'.__FUNCTION__.'() brackets[] not supported for "'.
											$this->_formFields[$fieldName]['type'].'" field',
															$this->_options['err_msg_level']);
						break;
					}
					/* check if field has values */
					if($err=$this->_checkErrors($fieldName,2,__FUNCTION__.'()')){ break; }
					$type=explode('[',$k);
					/* check storage keys */
					if($err=$this->_checkErrors($type[0],3,__FUNCTION__.'()')){ continue; }
					/* select only supports attributes[] with brackets[] */
					if($this->_formFields[$fieldName]['type']=='select' && $type[0]!='attributes')
					{
					...
					if($type[1]==']')
					{
						foreach($this->_formFields[$fieldName]['values'] as $key=>$arr)
						{
							$this->_addFieldParams($fieldName.'=>'.$key,$type[0],$v); 
						}
						continue;
					}
					else
					{
						...
						$this->_addFieldParams($fieldName.'=>'.$type[1],$type[0],$v); 
						continue;
					}
				}
				if($k=='values' || $k=='value'){ $this->addFieldValues($fieldName,$v); continue; }
				if($k=='events' && ($this->_formFields[$fieldName]['type']=='radiogroup' || 
								$this->_formFields[$fieldName]['type']=='checkboxgroup'))
				{
					if($err=$this->_checkErrors($fieldName,2,__FUNCTION__.'()')){ continue; }
					foreach($this->_formFields[$fieldName]['values'] as $key=>$arr)
					{
						$this->_addFieldParams($fieldName.'=>'.$key,$k,$v); 
					}
					continue;
				}
				if($err=$this->_checkErrors($k,3,__FUNCTION__.'()')){ continue; }
				$this->_addFieldParams($fieldName,$k,$v);
			}
			$this->_formFields[$fieldName]=$this->_addDefaultValues($this->_formFields[$fieldName]);
		}
		/**
		...
		* @param	array	$options		events to add to the input field
		* @deprecated
		*/
		...
		* @param	string		$fieldName	the name of the input field
		* @param	array|string	$options		attributes to add Ex:("class","id","style")
		* @deprecated
		*/
		...
		* @tutorial	PtcForms.cls#add_spacer
		*/
		...
		*/
		public function render($attributes=array(),$events=array())
		{
			...
			if(class_exists('PtcDebug',true))
			{
				...
				log_msg($this->_formFields,$attributes['id']." form fields",$this->_options['debug_category']); 
				$validator_debug=(!@$this->_validate['fields']) ? 
							$this->_validate : $this->_validate['fields'];
				log_msg($validator_debug,$attributes['id']." validator config.",$this->_options['debug_category']); 
		...
		* @tutorial	PtcForms.cls#validate_form
		...
		*/
		public function validate()
		{
			...
			if(is_array($this->_validate))
			{
				...
				if(class_exists("PtcDebug",true)){ log_msg($this->_validate,'validator result'); }
		...
		*/
		...
		/**
		* Add attributes to array of attributes for html elements 
		* @param	array|string	$attributes	array or string to add as attribute/s
		* @see		$_elAttributes
		* @tutorial	PtcForms.cls#add_el_attributes
		*/
		...
		* @see		$_labelStyles
		* @tutorial	PtcForms.cls#setLabelStyle		
		*/
		...
		* @see		$_inputStyles	
		* @tutorial	PtcForms.cls#setInputStyle
		*/
		...
		protected function _addClassValidator($fieldName,$options,$fieldType='default')
		{
			if($this->_options['add_class_validator'])
			{
				...
				{ 
					...
					{
						if(!@$this->_formFields[$fieldName]['values']){ break; }
						foreach($this->_formFields[$fieldName]['values'] as $key=>$arr)
						{
		...
		*/
		protected function _checkErrors($fieldName,$type,$function=null,$errType=null)
		{
			...
			switch($type)
			{
				...
				break;
				case 5:	# no fields in _formFields parameter
