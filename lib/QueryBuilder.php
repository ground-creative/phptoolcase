	...
	class QueryBuilder
	{
		...
		*/
		public function sanitize( $value )
		{
			...
			return addslashes( $value ); // could be done better
		}
		/**
		...
		*/
		public function update( $array , $id = null )
		{
			...
			if ( !$this->_where && !$id )
			{
				trigger_error( 'No id or where clause was specified for the update!' , E_USER_ERROR );
				return false;
			}
		...
		*/
		public function delete( $id = null )
		{
			...
			if ( !$this->_where && !$id )
			{
				trigger_error( 'No id or where clause was specified for the delete!' , E_USER_ERROR );
				return false;
			}
		...
		*/
		public function __call( $method , $args )
		{
			if ( false !== strpos( $method , 'where' ) ) // work with where clause
			{
				...
				if ( false !== strpos( $type , '_between' ) )	// build between clause
				{
					$type = $this->_addAndOR( $type );
					$this->_buildBetweenClause( $args[ 0 ] , @$args[ 1 ] , @$args[ 2 ] , $type );
				}
				else if ( false !== strpos( $type , '_in' ) ) 		// build in clause
				{
					$type = $this->_addAndOR( $type );
			...
			trigger_error( 'Called to undefined method "' . 
				get_called_class( ) . '::' . $method . '( )"!' , E_USER_ERROR );
		}
		/**
		...
		*/
		...
		/**
		* Checks if a raw value was added
		*/
		...
		*/
		...
		{ 
			if ( !defined( '_QUERY_BUILDER_RANDOM_ID_' ) )
			{ 
				define( '_QUERY_BUILDER_RANDOM_ID_' , mt_rand( 1000000 , 9999999 ) . '_' ); 
			}
		...
		*/
		...
		{
			if ( !class_exists( $event_class = $this->_namespace( ) ) ){ return; }
